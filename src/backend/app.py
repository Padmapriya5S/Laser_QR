...
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))  # E:\SIH_Prototype\
src_path = os.path.join(project_root, 'src')  # E:\SIH_Prototype\src
if project_root not in sys.path:
    sys.path.insert(0, project_root)
if src_path not in sys.path:
    sys.path.insert(0, src_path)
print(f"sys.path updated: {sys.path}")  # Debug path
...
try:
    from utils.qr_generator import generate_qrs  # Use relative import from src/utils
...
def sync_udm_tms_data():
    ...
    # Generate serials based on quantity, avoiding duplicates
    new_serials = []
    ...
    # Generate TMS data for new serials
    new_tms_data = []
    for serial in new_serials:
        if serial[0]:  # Ensure serial exists
            ...
            usage_days = (datetime(2025, 9, 18) - datetime.strptime(install_date, "%Y-%m-%d")).days
    ...
    conn.commit()
    generate_qrs(new_serials=[s[0] for s in new_serials])  # Generate QRs only for new serials
    conn.close()
